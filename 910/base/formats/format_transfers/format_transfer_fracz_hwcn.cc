...
Status GetDstDataAfterTransForFracZToHwcn(const TransArgs &args, TransResult &result,
  ...
  if (dst == nullptr) {
    ...
    REPORT_CALL_ERROR("E19999", "Failed to alloc the memory for dst buf %" PRId64 ", shape %s "
                      "when trans format from %s to %s",
                      total_size, ShapeToString(args.dst_shape).c_str(),
                      TypeUtils::FormatToSerialString(args.src_format).c_str(),
                      TypeUtils::FormatToSerialString(args.dst_format).c_str());
      ...
      for (int64_t c_idx = 0; c_idx < c; c_idx++) {
        ...
        for (int64_t n_idx = 0; n_idx < n; n_idx++) {
          ...
          if (ret != EOK) {
            ...
            REPORT_CALL_ERROR("E19999", "Failed to copy data from FracZ offset %" PRId64 " to HWCN[%" PRId64 ", "
...
Status FormatTransferFracZHwcn::TransFormat(const TransArgs &args, TransResult &result) {
  ...
  if (total_size <= 0) {
    ...
    REPORT_CALL_ERROR("E19999",  "Failed to get total size %" PRId64 " from "
