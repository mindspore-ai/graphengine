...
#ifndef ACLDVPP_DECODE_JPEG_H_
#define ACLDVPP_DECODE_JPEG_H_
...
*/
acldvppStatus acldvppDecodeJpegGetWorkspaceSize(const aclTensor* self, uint32_t channels,
    bool tryRecoverTruncated, aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor);

/**
* @brief acldvppDecodeJpeg的第二段接口，用于执行计算。
* @param [in] workspace: 在npu device侧申请的workspace内存起址。
* @param [in] workspaceSize: 在npu device侧申请的workspace大小，由第一段接口 acldvppDecodeJpegGetWorkspaceSize 获取。
