...
class OpUtils {
 public:
  ...
  ///
  /// @ingroup domi_omg
  /// @brief Converts a dimension array in one format to a dimension array in another format
  ...
  ///
  /// @ingroup domi_ome
  /// @brief Initializes the tensor description, which is used for input and output.
  ...
  ///
  /// @ingroup domi_ome
  /// @brief Initializes the tensor description for bias.
  /// @param [in] model_tensor Tensor information defined by the offline model
  /// @param [out]  cc_tensor Tensor definition used by CC
  /// @return SUCCESS success
  /// @return FAILED fail
  ...
  ///
  /// @brief Extract AIPP parameters from AttrDefMap and splice them with aipp_params.
  ...
  ///
  /// @ingroup domi_omg
  /// @brief Initialize the input and output description of the Data node that functions as the filter weight in the
  /// training network
...
class CceTensorDescriptor {
 public:
  ...
  /// @param[out] format format of tensor
  /// @return     return code
  ///
  Status GetFormat(ccTensorFormat_t *format);

  ///
  /// @brief      Obtains the size of a tensor.
