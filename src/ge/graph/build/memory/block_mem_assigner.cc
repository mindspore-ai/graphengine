...
Status BlockMemAssigner::AssignOutputMemoryWithReuse(const NodePtr &node, vector<int64_t> &ranges) {
  ...
  for (uint32_t i = 0; i < static_cast<uint32_t>(op_desc->GetOutputsSize()); i++) {
    ...
    if (l1_flag) {
      GELOGI("fusion: node[%s], output[%s], output memory type [%ld]", op_desc->GetName().c_str(),
...
void BlockMemAssigner::ResizeMemoryBlocks() {
  ...
  GELOGD("mem_offset_ exclude zero_copy_memory is %zu, p2p_mem_offset_ exclude zero_copy_memory is %zu.", mem_offset_,
         p2p_mem_offset_);
}
