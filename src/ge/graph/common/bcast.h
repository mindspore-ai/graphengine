...
class BCast {
 public:
  ...
  static Status ToIndexArray(const BCast::kVecInt &vec, Eigen::array<Eigen::DenseIndex, NDIMS> &ret) {
    if (vec.size() != NDIMS) {
      DOMI_LOGE("ToIndexArray failed. size of vector = %zu is not equal to NDIMS = %d.", vec.size(), NDIMS);
  ...
  Status BCastCompute(const std::vector<ConstGeTensorPtr> &input, std::vector<OutT> &v_output,
    ...
    ret = GenerateBcastInfo(TransShapeToDimVec(input[0]->GetTensorDesc()),
                            TransShapeToDimVec(input[1]->GetTensorDesc()));
    if (ret != domi::SUCCESS) {
  ...
  Status BCastComputeCheck(const std::vector<ConstGeTensorPtr> &input, std::vector<OutT> &v_output,
    ...
    // Only broadcast shape
    Status ret = GenerateBcastInfo(TransShapeToDimVec(input[0]->GetTensorDesc()),
                                   TransShapeToDimVec(input[1]->GetTensorDesc()));
    if (ret != SUCCESS) {
      GELOGE(ret, "Greater broadcasting failed.");
      return ret;
