...
Status ZeroCopyOffset::InitInputDataInfo(const vector<int64_t> &output_size_list,
                                         ...
                                         bool &fusion_flag) {
  GELOGI("[ZCPY] Start to InitInputDataInfo of %s, total_data_size is %ld, virtual_addr is 0x%lx",
  ...
  if (!fusion_flag) {
    ...
    GELOGI("[ZCPY] %s size is %ld, virtual_addr is 0x%lx.", op_desc->GetName().c_str(), output_size_list[kDataIndex],
    ...
    for (size_t index = 0; index < zero_copy_basic_offset_.size(); ++index) {
      if (zero_copy_basic_offset_.at(index) == virtual_addr_offset) {
        out_count++;
        int64_t out_offset =
          reinterpret_cast<int64_t>(virtual_addr_list[kDataIndex]) + zero_copy_relative_offset_.at(index);
        data_info_.emplace_back(data_size_, reinterpret_cast<void *>(out_offset));
        relative_offset_.emplace_back(zero_copy_relative_offset_.at(index));
        GELOGI("[ZCPY] virtual_addr: %p has been l2-fusion to %ld, need copy total_data_size is %ld.", basic_addr_,
               out_offset, data_size_);
      }
    }
  }
...
Status ZeroCopyOffset::InitOutputDataInfo(const vector<int64_t> &input_size_list,
  ...
  if (!fusion_flag) {
    ...
    relative_offset_.emplace_back(0);
    GELOGI("[ZCPY] %s size is %ld, virtual_addr is 0x%lx.", op_desc->GetName().c_str(), size, virtual_addr_list[idx]);
  } else {
    GELOGI("[ZCPY] set l2-fusion for %s.", op_desc->GetName().c_str());
    for (size_t index = 0; index < zero_copy_basic_offset_.size(); ++index) {
      if (zero_copy_basic_offset_.at(index) == virtual_addr_offset) {
        in_count++;
        int64_t in_offset = reinterpret_cast<int64_t>(virtual_addr_list[idx]) + zero_copy_relative_offset_.at(index);
        data_info_.emplace_back(data_size_, reinterpret_cast<void *>(in_offset));
        relative_offset_.emplace_back(zero_copy_relative_offset_.at(index));
        GELOGI("[ZCPY] virtual_addr: %p has been l2-fusion from %ld, need copy total_data_size is %ld.", basic_addr_,
               in_offset, data_size_);
      }
    }
  }
...
void ZeroCopyOffset::SetInputOutsideAddrs(const vector<int64_t> &output_offset_list, void *addr, const size_t &index,
  ...
  GELOGI("[ZCPY] Start to SetInputOutsideAddrs for virtual_addr 0x%lx.", addr);
  uint32_t out_count = 0;
  if (!fusion_flag) {
    GELOGI("[ZCPY] not set l2-fusion for virtual_adr 0x%lx.", addr);
    out_count++;
    ...
    GELOGI("[ZCPY] set l2-fusion for virtual_addr 0x%lx.", addr);
...
void ZeroCopyOffset::SetOutputOutsideAddrs(const int64_t &input_offset, const bool &fusion_flag, void *addr,
  ...
  GELOGI("[ZCPY] Start to SetOutputOutsideAddrs for virtual_addr 0x%lx.", addr);
  uint32_t out_count = 0;
  if (!fusion_flag) {
    GELOGI("[ZCPY] not set l2-fusion for virtual_addr 0x%lx.", addr);
    out_count++;
    std::map<const void *, std::vector<void *>> addr_mapping;
    addr_mapping[addr] = {};
    outside_addrs_.emplace_back(addr_mapping);
...
bool ZeroCopyOffset::SetOutsideAddrsValue(ZeroCopyTask &zero_copy_task, void *outside_addr, void *args, size_t offset) {
  ...
  for (uint32_t out_count = 0; out_count < GetAddrCount(); ++out_count) {
    ...
    if (args_addrs != addrs_mapping_list[out_count].end()) {
      ...
      GELOGI("[ZCPY] set copy input: virtual_addr: %p, task_addr: %p, args: %p, offset: %zu.", addr_val, args_val, args,
             offset);
