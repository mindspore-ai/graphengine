...
Status MappingSubgraphIndex(const ComputeGraphPtr &graph, const std::function<int(int data_index)> &input,
...
Status ParseSubgraphPostFnWhile(const string &subgraph_name, const ComputeGraphPtr &graph) {
  return MappingSubgraphIndex(
    graph, [](int data_index) { return data_index; },
...
Status DataPass::PostParseSubgraph(const ComputeGraphPtr &graph, const string &ir_name, const NodePtr &parent_node) {
  ...
  if (post_func_it == subgraph_handle.end()) {
    GELOGE(FAILED, "The subgraph post func for node %s type %s is null.", parent_node->GetName().c_str(),
