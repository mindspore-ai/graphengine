...
Status SetInputOutputOffsetPass::Run(ComputeGraphPtr graph) {
  ...
  for (auto &node : graph->GetDirectNode()) {
    ...
    if (!connect_input.empty()) {
      Status ret = SetInputOffset(graph, node);
      if (ret != SUCCESS) {
    ...
    if (!connect_output.empty()) {
      Status ret = SetOutputOffset(graph, node, connect_output);
      if (ret != SUCCESS) {
...
Status SetInputOutputOffsetPass::SetInputOffset(const ComputeGraphPtr &graph, const NodePtr &node) {
  ...
  bool get_input_continuous = ge::AttrUtils::GetBool(op_desc, ATTR_NAME_CONTINUOUS_INPUT, is_input_continuous);
  if (get_input_continuous && is_input_continuous) {
    GELOGW("current not support set offset for %s.", node->GetName().c_str());
  }
  return SUCCESS;
}

Status SetInputOutputOffsetPass::SetOutputOffsetForConcat(const ComputeGraphPtr &graph, const NodePtr &node) {
...
Status SetInputOutputOffsetPass::SetOutputOffsetForHcom(const ComputeGraphPtr &graph, const NodePtr &node,
                                                        const vector<int> &connect_output) {
...
Status SetInputOutputOffsetPass::SetOutputOffset(const ComputeGraphPtr &graph, const NodePtr &node,
                                                 const vector<int> &connect_output) {
  ...
  if (get_attr_no_task && attr_no_task) {
    ...
    if (is_input_continuous || buffer_fusion) {
      Status ret = SetOutputOffsetForConcat(graph, node);
      if (ret != SUCCESS) {
  ...
  bool get_output_continuous =
      ge::AttrUtils::GetBool(node->GetOpDesc(), ATTR_NAME_CONTINUOUS_OUTPUT, is_output_continuous);
  if (get_output_continuous && is_output_continuous) {
    Status ret = SetOutputOffsetForHcom(graph, node, connect_output);
    if (ret != SUCCESS) {
