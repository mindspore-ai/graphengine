...
#ifndef GE_HOST_AICPU_ENGINE_OPS_KERNEL_STORE_OP_ASSIGN_OP_H_
...
class AssignOp : public Op {
 public:
  AssignOp(const Node &node, RunContext &run_context) : Op(node, run_context) {}
  ~AssignOp() override = default;
  AssignOp &operator=(const AssignOp &op) = delete;
  AssignOp(const AssignOp &op) = delete;
...
#endif  // GE_HOST_AICPU_ENGINE_OPS_KERNEL_STORE_OP_ASSIGN_OP_H_
