...
#include "hybrid/node_executor/hostaicpu/kernel/assign_kernel.h"
...
Status AssignKernel::Compute(TaskContext& context) {
  GELOGI("AssignKernel [%s, %s] compute begin.", node_->GetName().c_str(), node_->GetType().c_str());
  ...
  if (value_tensor->GetSize() > ref_tensor->GetSize()) {
    GELOGE(INTERNAL_ERROR, "[%s] value_input_size=%zu, but ref_input_size=%zu.", node_->GetName().c_str(),
           value_tensor->GetSize(), ref_tensor->GetSize());
    return INTERNAL_ERROR;
  }

  GELOGI("[%s] value_input_data=%p, ref_input_size=%zu, value_input_size=%zu.", node_->GetName().c_str(),
         ref_tensor->GetSize(), ref_tensor->GetData(), value_tensor->GetSize());
  ...
  GE_CHK_STATUS_RET(context.SetOutput(kAssignRefOutputIndex, *ref_tensor), "[%s] Failed to set output.",
                    context.GetNodeName());

  GELOGI("AssignKernel [%s, %s] compute success.", node_->GetName().c_str(), node_->GetType().c_str());
  return SUCCESS;
}
