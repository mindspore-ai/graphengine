...
#include "hybrid/node_executor/hostaicpu/kernel/variable_kernel.h"
...
Status VariableKernel::Compute(TaskContext& context) {
  GELOGI("VariableKernel [%s, %s] compute begin.", node_->GetName().c_str(), node_->GetType().c_str());
  ...
  GELOGI("VariableKernel [%s, %s] compute success.", node_->GetName().c_str(), node_->GetType().c_str());
  return SUCCESS;
}
