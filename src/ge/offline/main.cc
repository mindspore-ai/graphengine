 ...
 public:
  ...
  static Status CheckFlags() {
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
      ge::CheckDisableReuseMemoryParamValid(to_string(FLAGS_disable_reuse_memory)) != ge::SUCCESS, ret = ge::FAILED,
      "check disable_reuse_memory failed!");
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(CheckFrameWorkValid(FLAGS_framework, FLAGS_weight) != ge::SUCCESS, ret = ge::FAILED,
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
      ...
      ErrorManager::GetInstance().ATCReportErrMessage(
        ...
        {"--insert_op_conf", FLAGS_insert_op_conf, "dynamic dims function does not support aipp"});
     ...
     */
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(FLAGS_model != "" && !ge::CheckInputPathValid(FLAGS_model, "--model"),
                                   ret = ge::FAILED, "model file %s not found!!", FLAGS_model.c_str());

    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(FLAGS_weight != "" && !ge::CheckInputPathValid(FLAGS_weight, "--weight"),
                                   ret = ge::FAILED, "weight file %s not found!!", FLAGS_weight.c_str());

    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(FLAGS_cal_conf != "" && !ge::CheckInputPathValid(FLAGS_cal_conf, "--cal_conf"),
                                   ret = ge::FAILED, "calibration config file %s not found!!", FLAGS_cal_conf.c_str());

    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
      FLAGS_op_name_map != "" && !ge::CheckInputPathValid(FLAGS_op_name_map, "--op_name_map"), ret = ge::FAILED,
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(!ge::CheckOutputPathValid(FLAGS_check_report, "--check_report"), ret = ge::FAILED,
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
      ...
      ret = ge::FAILED, "Input parameter[--save_original_model]'s value[%s] must be true or false.",
      FLAGS_save_original_model.c_str());
    GE_CHK_BOOL_EXEC(ge::CheckBufferOptimizeParamValid(FLAGS_buffer_optimize) == ge::SUCCESS, ret = ge::FAILED,
  ...
  static Status CheckConverJsonParamFlags() {
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(FLAGS_om == "",
                                   ...
                                   ret = ge::FAILED, "Input parameter[--om]'s value is empty!!");
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(FLAGS_json == "",
                                   ...
                                   ret = ge::FAILED, "Input parameter[--json]'s value is empty!!");

    // Check if the model path is valid
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(FLAGS_om != "" && !ge::CheckInputPathValid(FLAGS_om, "--om"), ret = ge::FAILED,
                                   "model file path is invalid: %s.", FLAGS_om.c_str());

    // Check whether the JSON path is valid
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(FLAGS_json != "" && !ge::CheckOutputPathValid(FLAGS_json, "--json"),
                                   ret = ge::FAILED, "json file path is invalid: %s.", FLAGS_json.c_str());
...
bool CheckMemInfo() {
  ...
  if ((current_mem_available > 0) && (current_mem_available < kMinAvailableMem)) {
    GELOGE(ge::PARAM_INVALID, "Current available mem [%lu] can not be smaller than [%lu] .", current_mem_available,
           kMinAvailableMem);
