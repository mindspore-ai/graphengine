...
Status SessionManager::Initialize(const std::map<std::string, std::string> &options) {
  if (init_flag_) {
    GELOGW("Session Manager has been intialized.");
    return SUCCESS;
  }
  init_flag_ = true;
  return SUCCESS;
...
Status SessionManager::Finalize() {
  if (!init_flag_) {
    GELOGW("Session Manager has not been intialized.");
    return SUCCESS;
  }
...
Status SessionManager::SetrtContext(rtContext_t rt_context) {
  GELOGI("set rt_context RT_CTX_NORMAL_MODE, devid:%u.", GetContext().DeviceId());
