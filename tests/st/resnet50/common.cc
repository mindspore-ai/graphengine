...
/// return:
///  true of false
bool compFp32WithTData(float *actualOutputData, std::string expectedDataFile, float rtol = 1e-05, float atol = 1e-08) {
  std::vector<uint64_t> dim_info;
  float *expectedOutputData = (float *)readTestDataFile(expectedDataFile, dim_info);
...
ge::Tensor genTensor(std::vector<int64_t> Tensorshape, Format format, DataType dt) {
  int size = 1;
  for (int i = 0; i < Tensorshape.size(); i++) {
    size = size * Tensorshape[i];
  }

  int dataType_size = SwitchDatatype(dt);

  size = abs(size * dataType_size);
  vector<uint8_t> dataValue;

  if (size == 0) {
    TensorDesc inputTensorDesc = TensorDesc(ge::Shape(Tensorshape), format, dt);
    inputTensorDesc.SetRealDimCnt(Tensorshape.size());
    Tensor genTensor = Tensor(inputTensorDesc, dataValue);
    return genTensor;
  }
...
ge::Tensor genTensor_withVaule(std::vector<int64_t> Tensorshape, float value) {
  int size = 1;
  for (int i = 0; i < Tensorshape.size(); i++) {
    size = size * Tensorshape[i];
  }

  float *dataValue = new float[size];
  for (int i = 0; i < size; i++) {
    *(dataValue + i) = value;
  }
  Tensor genGeTensor;
  TensorDesc inputTensorDesc = TensorDesc(ge::Shape(Tensorshape), FORMAT_NCHW);
  genGeTensor.SetTensorDesc(inputTensorDesc);
  genGeTensor.SetData((uint8_t *)dataValue, size * 4);
...
Tensor genTesnor_Shape_as_data(std::vector<int64_t> Tensorshape) {
  ...
  int size = Tensorshape.size();
  int32_t *tensor_data = new int32_t[size];
  std::cout << "shape tensor size:" << size << endl;
  for (int i = 0; i < size; i++) {
    *(tensor_data + i) = Tensorshape[i];
  }

  Tensor genTensor;
  TensorDesc inputTensorDesc = TensorDesc(ge::Shape({size}), FORMAT_NCHW, DT_INT32);
  genTensor.SetData((uint8_t *)tensor_data, size * GetDatTypeSize(dt));
  genTensor.SetTensorDesc(inputTensorDesc);
...
int RunGraph_initData(Graph &graph, string op_name, map<string, std::vector<int64_t>> attr_Test, string train_flag,
  ...
  if (attr_Test.find("input1") != attr_Test.end()) {
    Tensor inputTensor = genTensor(attr_Test["input1"]);
    input.push_back(inputTensor);
  }
  if (attr_Test.find("input2") != attr_Test.end()) {
    Tensor inputTensor = genTensor(attr_Test["input2"]);
    input.push_back(inputTensor);
  }
  if (attr_Test.find("input3") != attr_Test.end()) {
    Tensor inputTensor = genTensor(attr_Test["input3"]);
    input.push_back(inputTensor);
  }
...
ge::Status session_add_and_run_graph(ge::Session *session, uint32_t graphId, Graph &graph, std::vector<Tensor> inputs,
  ...
  ge::Status ret = session->AddGraph(graphId, graph);
  EXPECT_EQ(ret, ge::SUCCESS);
...
int buildConvGraph_new(Graph &graph, std::vector<TensorDesc> desc_var, std::vector<std::string> name_var, int flag,
                       Format format) {
  auto data_xShape = op::Data("xShape").set_attr_index(0);
  ...
  auto conv2d = op::Conv2D().set_input_x(data_xShape).set_input_filter(var).set_attr_strides({1, 1, 1, 1});
  ...
  if (flag >= 1) {
    conv2dgrad.set_input_x(data_xShape)
  ...
  std::vector<Operator> inputs{data_xShape};  // set all val
...
/// load bin data_fail
/// inputpath: path of bin data_file
/// shapes: the shape of Tensor
/// ft: the format of Tensor
/// dt: the dataType of Tensor
Tensor load_variable_input_data(string inputpath, std::vector<int64_t> shapes, Format ft, DataType dt) {
  vector<uint64_t> dimInfo_1;

  uint8_t *inputData = (uint8_t *)readTestDataFile(inputpath, dimInfo_1);  // common.h
  TensorDesc inputTensorDesc = TensorDesc(ge::Shape(shapes), ft, dt);
  inputTensorDesc.SetRealDimCnt(shapes.size());
  Tensor inputTensor = Tensor(inputTensorDesc, inputData, GetDatTypeSize(dt) * dimInfo_1[dimInfo_1[0] + 1]);
  return inputTensor;
}
