...
class ge_test_operator : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(ge_test_operator, tryGetInputDesc) {
  Operator Data("data0");
...
TEST_F(ge_test_operator, GetDynamicInputNum) {
  Operator Const("constNode");

  (void)Const.DynamicInputRegister("data", 2, 1);
  int num = Const.GetDynamicInputNum("data");
  EXPECT_EQ(num, 2);
}

TEST_F(ge_test_operator, InferFormatFuncRegister) {
  Operator add("add");
...
TEST_F(ge_test_operator, GetInferFormatFuncRegister) {
...
TEST_F(ge_test_operator, GetAllAttrNamesAndTypes) {
  Operator attr("attr");
