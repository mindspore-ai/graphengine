...
TEST_F(ge_test_tensor, originShapeFormat) {
  GeTensorDesc a;
...
TEST_F(ge_test_tensor, getShapeSize) {
...
TEST_F(ge_test_tensor, GeShapeToString1) {
...
TEST_F(ge_test_tensor, tensor) {
  GeShape s({1, 2, 3, 4});
  GeTensorDesc tensorDesc(s);
  std::vector<uint8_t> data({1, 2, 3, 4});
  GeTensor a;
  GeTensor b(tensorDesc);
  GeTensor c(tensorDesc, data);
  GeTensor d(tensorDesc, data.data(), data.size());
  ...
  GeTensor e(std::move(tensorDesc), std::move(data));
...
TEST_F(ge_test_tensor, test_TensorDesc_InvalidNull) {
  GeTensorDesc tensorDesc(nullptr, nullptr);
  EXPECT_EQ(tensorDesc.GetDataType(), DT_UNDEFINED);
  EXPECT_EQ(tensorDesc.GetFormat(), FORMAT_RESERVED);
  EXPECT_EQ(tensorDesc.MutableShape().shape_def_.GetProtoMsg(), nullptr);

  GeTensorDesc tensorDesc2;
  EXPECT_EQ(tensorDesc2.GetDataType(), DT_FLOAT);
  EXPECT_EQ(tensorDesc2.GetFormat(), FORMAT_ND);

  tensorDesc2.SetDataType(DT_DUAL_SUB_INT8);
  EXPECT_EQ(tensorDesc2.GetDataType(), DT_DUAL_SUB_INT8);

  CompressInfo info;
  EXPECT_EQ(TensorUtils::GetCmpsInfo(tensorDesc2, info), GRAPH_FAILED);
  TensorUtils::SetCmpsInfo(tensorDesc2, info);
  EXPECT_EQ(TensorUtils::GetCmpsInfo(tensorDesc2, info), GRAPH_SUCCESS);
...
TEST_F(ge_test_tensor, test_Tensor_InvalidNull) {
  ProtoMsgOwner msgOwner;
  GeTensor tensor(msgOwner, nullptr);
  ...
  auto tensorPtr = std::make_shared<GeTensor>(msgOwner, nullptr);
...
TEST_F(ge_test_tensor, test_Tensor_Utils_WeightSize) {
  ...
  auto tensorPtr = std::make_shared<GeTensor>();
  TensorUtils::SetWeightSize(tensorPtr->MutableTensorDesc(), 100);
  EXPECT_EQ(TensorUtils::GetWeightSize(tensorPtr), 100);
  EXPECT_TRUE(TensorUtils::GetWeightAddr(tensorPtr, buffer) != nullptr);
...
TEST_F(ge_test_tensor, test_Tensor_Valid) {
  ...
  // scalar 2
  TensorDesc tensorDesc7(Shape(), FORMAT_NCHW, DT_FLOAT);
  float data7 = 2;
  Tensor tensor7(tensorDesc7, (uint8_t *)&data7, sizeof(float));
  EXPECT_EQ(tensor7.IsValid(), GRAPH_SUCCESS);

  // string scalar
  TensorDesc tensorDesc8(Shape(), FORMAT_NCHW, DT_STRING);
  Tensor tensor8;
  tensor8.SetTensorDesc(tensorDesc8);
  ...
  // string vector
  TensorDesc tensorDesc9(Shape({2}), FORMAT_NCHW, DT_STRING);
  vector<string> data9 = {"A handsome boy write this code", "very handsome"};
  Tensor tensor9(tensorDesc9);
  EXPECT_EQ(tensor9.SetData(data9), GRAPH_SUCCESS);
  EXPECT_EQ(tensor9.IsValid(), GRAPH_SUCCESS);

  vector<string> emptyData9;
  EXPECT_EQ(tensor9.SetData(emptyData9), GRAPH_FAILED);
}

TEST_F(ge_test_tensor, test_Tensor_Invalid) {
  // Tensor(const TensorDesc &tensorDesc, const std::vector<uint8_t> &data)
  Shape shape({1, 1, 1});
  TensorDesc tensorDesc(shape);
  ...
  // Tensor()
  Tensor tensor4;
  tensor4.SetTensorDesc(tensorDesc);
  EXPECT_EQ(tensor4.IsValid(), GRAPH_SUCCESS);
  ...
  // scalar
  TensorDesc tensorDesc6(Shape(), FORMAT_NCHW, DT_FLOAT);
  uint8_t data6 = 2;
  Tensor tensor6(tensorDesc6, &data6, 1);
  EXPECT_EQ(tensor6.IsValid(), GRAPH_SUCCESS);
}
