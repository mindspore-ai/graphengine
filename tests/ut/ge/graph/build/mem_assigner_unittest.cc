...
class MemoryAssignerTest : public testing::Test {
 public:
  ge::OpDescPtr createOpWithWsSize(const string &name, int64_t wsByte, const string &type = "some") {
    ...
    workspaceBytes.push_back(wsByte);
    op_def->SetWorkspaceBytes(workspaceBytes);
    return op_def;
  }
...
TEST_F(MemoryAssignerTest, MemoryBlock_Resize_RealSizeList_is_empty) {
...
class MockBlockMemAssigner : public BlockMemAssigner {
 public:
  ...
  domi::Status GetMemoryRanges(std::vector<int64_t> &ranges) override { return domi::FAILED; }
};
}  // namespace ge

// when check GetMemoryRanges return fail, Assign return fail
TEST_F(MemoryAssignerTest, MockBlockMemAssigner_FAILED) {
  ge::ComputeGraphPtr graph = make_shared<ge::ComputeGraph>("");
  make_graph(graph);
  MockBlockMemAssigner mockAssigner(graph);

  EXPECT_EQ(mockAssigner.Assign(), domi::FAILED);
}
