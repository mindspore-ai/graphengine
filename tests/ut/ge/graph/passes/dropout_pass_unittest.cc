...
namespace ge {
class UTEST_graph_passes_dropout_pass : public Test {
 protected:
...
TEST_F(UTEST_graph_passes_dropout_pass, dropout_remove_succ) {
  ...
  DropOutPass dropOutPass;
  Status status = dropOutPass.Run(dropout_node);
  ...
  status = dropOutPass.Run(node);
  EXPECT_EQ(PARAM_INVALID, status);
}

TEST_F(UTEST_graph_passes_dropout_pass, dropout_remove_fail1) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(UTEST_graph_passes_dropout_pass, dropout_square) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  NodePtr dropout_node = AddNode(graph, "dropout", DROPOUT);
