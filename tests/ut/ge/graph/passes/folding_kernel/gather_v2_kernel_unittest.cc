...
class UTEST_graph_passes_folding_kernel_gather_v2_kernel : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT32_axis_0_a) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
  ...
  ConstGeTensorPtr tensor_2 =
      make_shared<GeTensor>(tensor_desc_2, (uint8_t *)axis_vec.data(), axis_vec.size() * sizeof(int32_t));
  ...
  auto real_output_shape = v_output[0]->GetTensorDesc().GetShape().GetDims();
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT32_axis_0_b) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT64_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
  ...
  ConstGeTensorPtr tensor_2 =
      make_shared<GeTensor>(tensor_desc_2, (uint8_t *)axis_vec.data(), axis_vec.size() * sizeof(int64_t));
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT32_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT32_axis_0_1) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT32_axis_1) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT32_axis_2) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT32_axis_3) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT8_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, INT16_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, UINT8_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, UINT16_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, UINT32_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, UINT64_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, DOUBLE_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, FLOAT16_axis_0) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, abnormal_test_datatype_not_support) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, abnormal_test_indices_over_one) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, abnormal_test_axis_over_three) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...
TEST_F(UTEST_graph_passes_folding_kernel_gather_v2_kernel, abnormal_test) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("GatherV2", "GatherV2");
...

  {
    ...
    status = kernel->Compute(op_desc_ptr, input_1, v_output);
    EXPECT_NE(ge::SUCCESS, status);
    vector<ConstGeTensorPtr> input_2 = {nullptr, nullptr, nullptr};
    status = kernel->Compute(op_desc_ptr, input_2, v_output);
    EXPECT_NE(ge::SUCCESS, status);
    ConstGeTensorPtr tensor_11 = make_shared<GeTensor>(tensor_desc_1, (uint8_t *)data_vec_1.data(), 0);
    vector<ConstGeTensorPtr> input_11 = {tensor_0, tensor_11, tensor_2};
    status = kernel->Compute(op_desc_ptr, input_11, v_output);
    EXPECT_NE(ge::SUCCESS, status);

    GeTensorDesc tensor_desc_3(GeShape({1, 2, 3, 4}), FORMAT_NHWC, DT_INT32);
    ConstGeTensorPtr tensor_3 =
        make_shared<GeTensor>(tensor_desc_3, (uint8_t *)axis_vec.data(), axis_vec.size() * sizeof(int32_t));
    vector<ConstGeTensorPtr> input_3 = {tensor_0, tensor_1, tensor_2, tensor_2};
    status = kernel->Compute(op_desc_ptr, input_3, v_output);
    EXPECT_NE(ge::SUCCESS, status);
    GeTensorDesc tensor_desc_indices_1(GeShape(indices_shape), FORMAT_NHWC, DT_UINT32);
    ConstGeTensorPtr tensor_22 =
        make_shared<GeTensor>(tensor_desc_indices_1, (uint8_t *)data_vec_1.data(), data_vec_1.size() * sizeof(int32_t));
    vector<ConstGeTensorPtr> input_4 = {tensor_0, tensor_22, tensor_2};
    status = kernel->Compute(op_desc_ptr, input_4, v_output);
    EXPECT_NE(ge::SUCCESS, status);
    GeTensorDesc tensor_desc_axis_1(GeShape(indices_shape), FORMAT_NHWC, DT_UINT32);
    ConstGeTensorPtr tensor_33 =
        make_shared<GeTensor>(tensor_desc_axis_1, (uint8_t *)axis_vec.data(), axis_vec.size() * sizeof(int32_t));
    vector<ConstGeTensorPtr> input_5 = {tensor_0, tensor_33, tensor_2};
    status = kernel->Compute(op_desc_ptr, input_5, v_output);
    EXPECT_NE(ge::SUCCESS, status);
    ...
    status = kernel->Compute(op_desc_ptr, input_6, v_output);
    EXPECT_NE(ge::SUCCESS, status);
    ...
    ConstGeTensorPtr tensor_55 =
        make_shared<GeTensor>(tensor_desc_55, (uint8_t *)data_vec_3.data(), data_vec_3.size() * sizeof(int64_t));
    vector<ConstGeTensorPtr> input_7 = {tensor_0, tensor_55, tensor_2};
    status = kernel->Compute(op_desc_ptr, input_7, v_output);
    EXPECT_NE(ge::SUCCESS, status);
  }
}