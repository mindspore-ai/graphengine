...
class UTEST_graph_passes_folding_kernel_maximum_kernel : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_int_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
...
TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_int_success_2) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  AttrUtils::SetInt(op_desc_ptr, ATTR_NAME_T, (int64_t)DT_INT32);
...
TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_scalar_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  AttrUtils::SetInt(op_desc_ptr, ATTR_NAME_T, (int64_t)DT_INT32);
...
TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_int8_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  ...
  Status status = kernel->Compute(op_desc_ptr, input, v_output);
  EXPECT_EQ(status, SUCCESS);
  EXPECT_EQ(v_output[0]->GetData().size(), 96);  // 2*2*4*3*2
}

TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_int16_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  ...
  Status status = kernel->Compute(op_desc_ptr, input, v_output);
  EXPECT_EQ(status, SUCCESS);
  EXPECT_EQ(v_output[0]->GetData().size(), 192);  // 2*2*4*3*2*2
}

TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_int64_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  ...
  EXPECT_EQ(v_output[0]->GetData().size(), 768);  // 2*2*4*3*2*8
}

TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_uint8_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
...
TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_uint16_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
...
TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_uint32_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  ...
  Status status = kernel->Compute(op_desc_ptr, input, v_output);
  EXPECT_EQ(status, SUCCESS);
  EXPECT_EQ(v_output[0]->GetData().size(), 384);  // 2*2*4*3*2*4
}

TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_uint64_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
...
TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_float16_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  ...
  Status status = kernel->Compute(op_desc_ptr, input, v_output);
  EXPECT_EQ(status, SUCCESS);
  EXPECT_EQ(v_output[0]->GetData().size(), 8);  // 4*2
}

TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_float_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  ...
  Status status = kernel->Compute(op_desc_ptr, input, v_output);
  EXPECT_EQ(status, SUCCESS);
  EXPECT_EQ(v_output[0]->GetData().size(), 16);  // 4*4
}

TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_double_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  ...
  Status status = kernel->Compute(op_desc_ptr, input, v_output);
  EXPECT_EQ(status, SUCCESS);
  EXPECT_EQ(v_output[0]->GetData().size(), 32);  // 4*8
}

TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_errtype_fail) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
...
TEST_F(UTEST_graph_passes_folding_kernel_maximum_kernel, maximum_optimizer_different_type) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("Maximum", MAXIMUM);
  vector<bool> is_input_const_vec = {true, true};
  op_desc_ptr->SetIsInputConst(is_input_const_vec);
  AttrUtils::SetInt(op_desc_ptr, ATTR_NAME_T, (int64_t)DT_INT32);
