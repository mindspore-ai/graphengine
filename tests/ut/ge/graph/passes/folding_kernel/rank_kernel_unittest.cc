...
class UTEST_graph_passes_folding_kernel_rank_kernel : public testing::Test {
 protected:
  void SetUp() {
    graph = std::make_shared<ge::ComputeGraph>("default");
    op_desc_ptr = make_shared<OpDesc>("Rank", RANK);
...
TEST_F(UTEST_graph_passes_folding_kernel_rank_kernel, rank_is_1) {
...
TEST_F(UTEST_graph_passes_folding_kernel_rank_kernel, rank_is_3) {
...
TEST_F(UTEST_graph_passes_folding_kernel_rank_kernel, invalid_case_input_size_is_0) {
  std::vector<GeTensorPtr> v_output;
  Status status = kernel->Compute(node, v_output);
...
TEST_F(UTEST_graph_passes_folding_kernel_rank_kernel, invalid_case_input_size_is_more_than_1) {
  vector<int64_t> dims_vec_0 = {4, 2, 2};
  GeTensorDesc tensor_desc_0(GeShape(dims_vec_0), FORMAT_NCHW, DT_FLOAT);
  op_desc_ptr->AddInputDesc(tensor_desc_0);
  op_desc_ptr->AddInputDesc(tensor_desc_0);
