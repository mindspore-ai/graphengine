...
class UTEST_folding_kernel_rsqrt_kernel : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(UTEST_folding_kernel_rsqrt_kernel, rsqrt_optimizer_success) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("RSQRT", RSQRT);
  ...
  float *outdata = (float *)v_output[0]->GetData().data();
...
TEST_F(UTEST_folding_kernel_rsqrt_kernel, rsqrt_optimizer_has_zero) {
  OpDescPtr op_desc_ptr = make_shared<OpDesc>("RSQRT", RSQRT);
