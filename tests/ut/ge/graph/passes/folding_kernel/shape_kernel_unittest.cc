...
class UTEST_graph_passes_folding_kernel_shape_kernel : public testing::Test {
 protected:
  void SetUp() { init(); }
 ...
 private:
  void init() {
    ...
    op_desc_ptr_ = make_shared<OpDesc>("Shape", SHAPE);
...
TEST_F(UTEST_graph_passes_folding_kernel_shape_kernel, shape_optimizer_success) {
...
TEST_F(UTEST_graph_passes_folding_kernel_shape_kernel, shape_data_int32) {
  ...
  if (status == ge::SUCCESS) {
    ...
    EXPECT_EQ(v_output[0]->GetData().size(), sizeof(int32_t) * dims_vec_0.size());
    EXPECT_EQ(*(int32_t *)(v_output[0]->GetData().data()), 8);
  }
}

TEST_F(UTEST_graph_passes_folding_kernel_shape_kernel, shape_data_int64) {
  vector<int64_t> dims_vec_0 = {8, 2};
  GeTensorDesc tensor_desc_0(GeShape(dims_vec_0), FORMAT_NCHW, DT_FLOAT);
...
TEST_F(UTEST_graph_passes_folding_kernel_shape_kernel, shape_input_size_fail) {
  GeTensorDesc tensor_desc_out(GeShape(), FORMAT_NCHW, DT_INT64);
  op_desc_ptr_->AddOutputDesc(tensor_desc_out);
