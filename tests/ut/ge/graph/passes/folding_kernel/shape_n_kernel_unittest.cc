...
class UTEST_graph_passes_folding_kernel_shape_n_kernel : public testing::Test {
 protected:
  void SetUp() { init(); }
 ...
 private:
  void init() {
    ...
    op_desc_ptr_ = make_shared<OpDesc>("ShapeN", SHAPEN);
...
TEST_F(UTEST_graph_passes_folding_kernel_shape_n_kernel, size_optimizer_success) {
...
TEST_F(UTEST_graph_passes_folding_kernel_shape_n_kernel, shape_n_data_int32) {
  ...
  Status status = kernel_->Compute(node_, v_output);
  EXPECT_EQ(ge::SUCCESS, status);
  if (status == ge::SUCCESS) {
    EXPECT_EQ(v_output[1]->GetTensorDesc().GetDataType(), DT_INT32);
    EXPECT_EQ(v_output[1]->GetTensorDesc().GetShape().GetDimNum(), 1);
    EXPECT_EQ(v_output[1]->GetData().size(), sizeof(int32_t) * dims_vec_1.size());
    EXPECT_EQ(*(int32_t *)(v_output[1]->GetData().data()), 3);
  }
}

TEST_F(UTEST_graph_passes_folding_kernel_shape_n_kernel, shape_n_data_int64) {
  vector<int64_t> dims_vec_0 = {8, 2};
  GeTensorDesc tensor_desc_0(GeShape(dims_vec_0), FORMAT_NCHW, DT_FLOAT);
  ...
  EXPECT_EQ(ge::SUCCESS, status);
  if (status == ge::SUCCESS) {
    EXPECT_EQ(v_output[0]->GetTensorDesc().GetDataType(), DT_INT64);
    EXPECT_EQ(v_output[0]->GetTensorDesc().GetShape().GetDimNum(), 1);
    EXPECT_EQ(v_output[0]->GetData().size(), sizeof(int64_t) * dims_vec_0.size());
    EXPECT_EQ(*(int64_t *)(v_output[0]->GetData().data()), 8);
  }
}

TEST_F(UTEST_graph_passes_folding_kernel_shape_n_kernel, shape_n_2_in_1_out) {
  vector<int64_t> dims_vec_0 = {8, 2};
  GeTensorDesc tensor_desc_0(GeShape(dims_vec_0), FORMAT_NCHW, DT_FLOAT);
  TensorUtils::SetRealDimCnt(tensor_desc_0, dims_vec_0.size());
  op_desc_ptr_->AddInputDesc(tensor_desc_0);
