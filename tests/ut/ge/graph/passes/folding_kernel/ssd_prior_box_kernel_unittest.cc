...
class UTEST_graph_passes_folding_kernel_ssd_priorbox_kernel : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(UTEST_graph_passes_folding_kernel_ssd_priorbox_kernel, compute_failed) {
...
TEST_F(UTEST_graph_passes_folding_kernel_ssd_priorbox_kernel, compute_success) {
...
TEST_F(UTEST_graph_passes_folding_kernel_ssd_priorbox_kernel, aspectRatio_noflip_success) {
  ComputeGraphPtr compute_graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(UTEST_graph_passes_folding_kernel_ssd_priorbox_kernel, aspectratio_multiSize_success) {
  ComputeGraphPtr compute_graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(UTEST_graph_passes_folding_kernel_ssd_priorbox_kernel, multi_variance_success) {
  ComputeGraphPtr compute_graph = std::make_shared<ComputeGraph>("test");
...
TEST_F(UTEST_graph_passes_folding_kernel_ssd_priorbox_kernel, all_success) {
  int num_priors = 0;
  int dim_size = 0;
...
TEST_F(UTEST_graph_passes_folding_kernel_ssd_priorbox_kernel, param_invalid) {
  int num_priors = 0;
  int dim_size = 0;
  SsdPriorboxKernel kernal;
