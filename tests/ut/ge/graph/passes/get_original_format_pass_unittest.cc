...
class UTEST_graph_passes_get_original_format_pass : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(UTEST_graph_passes_get_original_format_pass, no_transpose_success) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  make_graph(graph);

  ge::GetOriginalFormatPass get_format_pass;
  domi::Status status = get_format_pass.Run(graph);
...
TEST_F(UTEST_graph_passes_get_original_format_pass, infered_format_need_to_reset_success) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  make_graph(graph);
...
TEST_F(UTEST_graph_passes_get_original_format_pass, infered_format_need_to_reset_success2) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
