...
class UTEST_graph_passes_guarantee_const_pass : public testing::Test {
 protected:
  void SetUp() { init(); }
 ...
 private:
  void init() { guaranteeConstOpRemovePass = new ::ge::GuaranteeConstPass(); }

  void destory() {
    delete guaranteeConstOpRemovePass;
    guaranteeConstOpRemovePass = NULL;
  }

 protected:
  ge::GuaranteeConstPass *guaranteeConstOpRemovePass;
  ...
  NodePtr init_node(ComputeGraphPtr graph, vector<int64_t> dims_vec, vector<int32_t> data_vec, bool isMultiInput,
    ...
    ge::AttrUtils::SetTensor(in_op_def, ge::ATTR_NAME_WEIGHTS, constTensor);
    ...
    if (isMultiInput) {
      ...
      ge::ConstGeTensorPtr constTensor_another = std::make_shared<GeTensor>(
          another_tensor_desc, (uint8_t *)&data_vec_another[0], data_vec_another.size() * sizeof(int32_t));
      ge::AttrUtils::SetTensor(another_in_op_def, ge::ATTR_NAME_WEIGHTS, constTensor_another);
...
TEST_F(UTEST_graph_passes_guarantee_const_pass, not_changed) {
  ...
  ge::Status ret = guaranteeConstOpRemovePass->Run(node);
  EXPECT_EQ(domi::SUCCESS, ret);
}

TEST_F(UTEST_graph_passes_guarantee_const_pass, get_origenal_type_fail) {
...
TEST_F(UTEST_graph_passes_guarantee_const_pass, Int32Success_6) {
  // input tensor
  string type = GUARANTEECONST;
  vector<int64_t> dims_vec = {6};
  vector<int32_t> data_vec = {1, 2, 3, 4, 5, 6};
  ...
  ge::Status output = guaranteeConstOpRemovePass->Run(node);
  EXPECT_EQ(ge::SUCCESS, output);
}

TEST_F(UTEST_graph_passes_guarantee_const_pass, Int32Success_2_3) {
  // input tensor
  string type = GUARANTEECONST;
...
TEST_F(UTEST_graph_passes_guarantee_const_pass, IsolateNodeFailed) {
  // input tensor
  string type = GUARANTEECONST;
  vector<int64_t> dims_vec = {2, 3};
  vector<int32_t> data_vec = {1, 2, 3, 4, 5, 6};
...
TEST_F(UTEST_graph_passes_guarantee_const_pass, ir_infer_shape) {
