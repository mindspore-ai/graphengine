...
class UTEST_graph_passes_merge_pass : public testing::Test {
 protected:
  UTEST_graph_passes_merge_pass() {
...
TEST_F(UTEST_graph_passes_merge_pass, NullInput) {
  NodePtr node = nullptr;
  auto ret = pass_.Run(node);
  EXPECT_EQ(ret, PARAM_INVALID);
}

TEST_F(UTEST_graph_passes_merge_pass, FilterNonMergeNode) {
  auto node = NewNode("Op1", CONSTANT, 0, 1);
  auto ret = pass_.Run(node);
  EXPECT_EQ(ret, SUCCESS);
}

TEST_F(UTEST_graph_passes_merge_pass, InvalidMergeNode) {
...
///       |
///   NetOutput
TEST_F(UTEST_graph_passes_merge_pass, MultipleInputs) {
...
///     \  |   /
///     NetOutput
TEST_F(UTEST_graph_passes_merge_pass, EmptyInputCutBranchMeetNetOutputWithDataAnchor) {
...
///     \  |    :
///     NetOutput
TEST_F(UTEST_graph_passes_merge_pass, EmptyInputCutBranchMeetNetOutputWithControlAnchor) {
...
TEST_F(UTEST_graph_passes_merge_pass, EmptyInputCutBranch) {
  ///      Merge
  ///        |  \
  ///        |   \
  ///  Op1  Op2 Merge2
...
TEST_F(UTEST_graph_passes_merge_pass, SingleNonConstInput) {
  ///      Op1
  ///       |
  ///     Merge
  ///      / \
...
TEST_F(UTEST_graph_passes_merge_pass, SingleConstInput) {
  ///     Const
  ///       |
  ///     Merge    Pass      Const
  ///      / \     ===>      /   \
...
TEST_F(UTEST_graph_passes_merge_pass, SingleConstInputValueIndexTwoOutNodes) {
  ///     Const
  ///       |
  ///     Merge        Pass      Const
  ///     /    |       ===>      /   \(control anchor)
...
TEST_F(UTEST_graph_passes_merge_pass, SingleConstInputValueIndexTwoOutNodes1) {
  ///     Const
  ///       |
  ///     Merge        Pass      Const
  ///     /    |       ===>      /   \(control anchor)
...
TEST_F(UTEST_graph_passes_merge_pass, ConstWithControlInput) {
  ///     Switch
  ///       |
  ///    Identity
  ///       .
