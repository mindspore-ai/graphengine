...
class SuccessGraphPass : public GraphPass {
  Status Run(ComputeGraphPtr graph) { return domi::SUCCESS; }
};

class NotChangedGraphPass : public GraphPass {
  Status Run(ComputeGraphPtr graph) { return domi::NOT_CHANGED; }
};

class ErrorGraphPass : public GraphPass {
  Status Run(ComputeGraphPtr graph) { return domi::FAILED; }
};

class UTEST_graph_passes_pass_manager_pass : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(UTEST_graph_passes_pass_manager_pass, all_pass_success) {
  PassManager manager;
...
TEST_F(UTEST_graph_passes_pass_manager_pass, graph_pass_success) {
  ComputeGraphPtr graph = CreatePadGraph();
...
TEST_F(UTEST_graph_passes_pass_manager_pass, graph_pass_not_changed) {
  ComputeGraphPtr graph = CreatePadGraph();
...
TEST_F(UTEST_graph_passes_pass_manager_pass, graph_pass_error) {
  ComputeGraphPtr graph = CreatePadGraph();
  ErrorGraphPass pass;
  vector<GraphPass *> passes = {&pass};
  Status status = PassManager::Run(graph, passes);
