...
class UTEST_graph_passes_pass_utils : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(UTEST_graph_passes_pass_utils, set_out_node_weight) {
...
TEST_F(UTEST_graph_passes_pass_utils, is_constant_null) {
  ge::NodePtr node = nullptr;
  bool ret = PassUtils::IsConstant(node);
  EXPECT_EQ(false, ret);
}

TEST_F(UTEST_graph_passes_pass_utils, get_in_data_node_fail) {
  ge::NodePtr node = nullptr;
...
TEST_F(UTEST_graph_passes_pass_utils, get_unique_in_data_anchor_index_failed) {
  int invalid_index = -1;
  ge::NodePtr node = nullptr;
...
TEST_F(UTEST_graph_passes_pass_utils, unlink_node_with_ctrl_copy_fail) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  // relu
  ge::NodePtr node_relu = NodeBuilder("relu", RELU)
                              .AddInputDesc({2, 2, 2, 2}, FORMAT_NCHW, DT_FLOAT)
...
TEST_F(UTEST_graph_passes_pass_utils, null_input) {
