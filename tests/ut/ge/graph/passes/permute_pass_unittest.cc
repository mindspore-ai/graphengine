...
class UTEST_graph_passes_permute_pass : public testing::Test {
 protected:
  void SetUp() {}
...
TEST_F(UTEST_graph_passes_permute_pass, transpose_and_conv3) {
  ComputeGraphPtr graph = CreatePadGraph();

  ge::PermutePass permute_pass;
  domi::Status status = permute_pass.Run(graph);
