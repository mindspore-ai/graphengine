...
class UTEST_graph_passes_stop_gradient_pass : public testing::Test {
 protected:
  void SetUp() { init(); }
...
TEST_F(UTEST_graph_passes_stop_gradient_pass, success) {
...
TEST_F(UTEST_graph_passes_stop_gradient_pass, not_changed) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  string type = SIZE;
  NodePtr node = init_node(graph, type);
  ge::Status ret = pass_->Run(node);
...
TEST_F(UTEST_graph_passes_stop_gradient_pass, get_origenal_type_fail) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  string type = STOPGRADIENT;
  NodePtr node = init_node(graph, type);
...
TEST_F(UTEST_graph_passes_stop_gradient_pass, size_check_fail) {
...
TEST_F(UTEST_graph_passes_stop_gradient_pass, ir_infer_shape) {
