...
namespace ge {
class UTEST_graph_passes_unused_const_pass : public Test {
 protected:
...
TEST_F(UTEST_graph_passes_unused_const_pass, unused_const_remove_succ) {
  ...
  UnusedConstPass unusedConstPass;
  Status status = unusedConstPass.Run(unused_const_node);
  EXPECT_EQ(SUCCESS, status);
  NodePtr found_node = graph->FindNode("unusedConst");
  EXPECT_EQ(nullptr, found_node);
}

TEST_F(UTEST_graph_passes_unused_const_pass, unused_const_remove_failed) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
